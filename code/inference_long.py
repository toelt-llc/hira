...
def inference_video(args, file, video_save_path):
    ...
    elif args.model_name == 'RealESRNet_x4plus_latest':
        model = RRDBNet(num_in_ch=3, num_out_ch=3, num_feat=64, num_block=23, num_grow_ch=32, scale=4)
        netscale = 4
...
def split_video(file, output_folder, duration=120):
    video_name = osp.splitext(os.path.basename(file))[0]
    split_folder = osp.join(output_folder, video_name)
    os.makedirs(split_folder, exist_ok=True)

    split_command = f'ffmpeg -i {file} -c copy -map 0 -segment_time {duration} -f segment {osp.join(split_folder, f"{video_name}_part%03d.mp4")}'
    os.system(split_command)

    return glob.glob(osp.join(split_folder, f"{video_name}_part*.mp4"))
...
def main():
    ...
    if mimetypes.guess_type(args.input)[0] is not None and mimetypes.guess_type(args.input)[0].startswith('video'):
        ...
        if meta['duration'] > 120:
            logger.info(f"Video duration exceeds 2 minutes. Splitting into parts.")
            parts = split_video(args.input, args.output)
            for part in parts:
                run(args, part)
        else:
            run(args, args.input)
        ...
        for file in glob.glob(os.path.join(args.input, '*')):
            if file.lower().endswith(('.mp4')):
                meta = get_video_meta_info(file)
                if meta['duration'] > 120:
                    logger.info(f"Video duration exceeds 2 minutes. Splitting into parts.")
                    parts = split_video(file, args.output)
                    for part in parts:
                        run(args, part)
                else:
                    run(args, file)
                ...
                if result == 0:
                    meta = get_video_meta_info(mp4_path)
                    if meta['duration'] > 120:
                        logger.info(f"Video duration exceeds 2 minutes. Splitting into parts.")
                        parts = split_video(mp4_path, args.output)
                        for part in parts:
                            run(args, part)
                    else:
                        run(args, mp4_path)
